(function(root,factory){
  ...
  if(root) root.ShadowScoringV53=api;
  ...
  function scoreSentence(manifest,detTokens,previous=null){
    ...
    const core=results.filter(w=>w.scorable),reliableCore=core.filter(w=>w.state!=='uncertain'),lowReliable=reliableCore.filter(w=>w.score<70&&w.state==='suspect');
    const uncertain=core.filter(w=>w.state==='uncertain'),confirmed=core.flatMap(w=>w.confirmedCritical.map(c=>({word:w.raw,...c}))),suspected=core.flatMap(w=>w.criticalSuspects.filter(c=>!c.confirmed).map(c=>({word:w.raw,...c})));
    // Uncertain acoustic evidence receives a neutral floor rather than being treated as proof of bad pronunciation.
    const clarity=Math.round(mean(core.map(w=>w.state==='uncertain'?Math.max(76,w.score):(w.criticalSuspects.length&&!w.confirmedCritical.length?Math.max(78,w.score):w.score)))||0);
    const under70Ratio=reliableCore.length?lowReliable.length/reliableCore.length:0,lowReliableCount=lowReliable.length,allowedLowCount=reliableCore.length?Math.max(1,Math.floor(reliableCore.length*.30)):0;
    const uncertaintyRatio=core.length?uncertain.length/core.length:1;
    return {words:results,clarity,under70Ratio,lowReliableCount,allowedLowCount,uncertaintyRatio,reliableCoreCount:reliableCore.length,coreCount:core.length,confirmedCritical:confirmed,suspectedCritical:suspected,detectedIPA:det.join(' '),segmentationCost:Math.round(seg.cost*100)/100,leadingPhones:seg.leadingPhones,trailingPhones:seg.trailingPhones};
  }

  function gate(result){
    if(!result||result.overall==null)return {pass:false,inconclusive:false,reasons:['No scored attempt yet.']};
    const reasons=[];
    if(result.uncertaintyRatio>.45) reasons.push(`Too much uncertain acoustic evidence (${Math.round(result.uncertaintyRatio*100)}%).`);
    if(result.overall<80) reasons.push(`Overall ${result.overall} < 80.`);
    if(result.clarity<78) reasons.push(`Pronunciation ${result.clarity} < 78.`);
    if((result.lowReliableCount||0)>(result.allowedLowCount||0)) reasons.push(`${result.lowReliableCount} reliable core words are below 70; this sentence allows ${result.allowedLowCount} before the low-word gate blocks.`);
    if((result.confirmedCritical||[]).length) reasons.push(`${result.confirmedCritical.length} critical pronunciation issue(s) repeated on a second valid attempt.`);
    const inconclusive=result.uncertaintyRatio>.45;
    return {pass:reasons.length===0,inconclusive,reasons};
  }

  return {version:'5.3-evidence-calibrated',phNorm,phEq,substitutionCost,localAlign,segmentSentence,scoreSentence,gate};
});
